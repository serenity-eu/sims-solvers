...
class InstanceSIMS(InstanceGeneric):
    ...
    def __init__(self, minizinc_instance):
        super().__init__(
            is_minizinc=False,
            problem_name=constants.Problem.SATELLITE_IMAGE_SELECTION_PROBLEM.value,
        )
        self.images, self.clouds = self.correct_starting_indexes(
            minizinc_instance["images"], minizinc_instance["clouds"]
        )
        ...
        self.cloud_covered_by_image, self.clouds_id_area = (
            self.get_clouds_covered_by_image()
        )
        ...
        for i in range(len(self.clouds)):
            ...
            for cloud_id in image_cloud_set:
                ...
                for j in range(len(self.images)):
                    if i != j:
                        if (
                            cloud_id in self.images[j]
                            and cloud_id not in self.clouds[j]
                        ):  # the area of the cloud is covered by image j, and it is not cloudy in j
